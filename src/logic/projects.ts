...
export const addProjects = async ( req: Request, res: Response): Promise<Response> => {
    try {
      const { name, description, estimatedTime, repository, startDate, developerId }: Project = req.body;
      ...
      const queryConfig: QueryConfig = {
          ...
          values: [name, description, estimatedTime, repository, startDate, developerId],
        };
...
export const addTechnologiesProjects = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const insertProjectTechnologyQueryString = format(
        ...
        VALUES 
        ($1, (SELECT id FROM technologies WHERE name = $2), NOW());
      `
      );
      ...
      const selectProjectQueryString = format(
        ...
        SELECT
          ...
          projects.estimated_time AS projectEstimatedTime,
